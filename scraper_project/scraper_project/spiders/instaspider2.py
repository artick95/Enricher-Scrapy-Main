...
class InstaSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for link in le.extract_links(response):
            ...
            try:
                # This is the regex pattern for an email
                email_re = r'[A-Za-z0-9\.\+_-]+@[A-Za-z0-9\._-]+\.[a-zA-Z]*'
                email_links_regex = re.findall(email_re, response.text)
                links['EmailRegexFound'] += email_links_regex
            except Exception as e:
                self.logger.error(f"Error extracting emails by regex from {response.url}. Error: {e}")
